...
- [x] `src/ledgerguard/d1_rule_learning/propose.py` — from one human-resolved exception,
      generalizes a candidate rule spec: a narration transform (best-matching known merchant
      token, a conservative scope), a match key (`single_net_amount_exact`), and a widened date
      tolerance (only ever widens the built-in [2,4]-day window, never narrows it).
- [x] `src/ledgerguard/d1_rule_learning/validate.py` — replays a candidate against train+
      validation history, respecting the real firing order (built-ins get first refusal), never
      touching holdout.
- [x] `src/ledgerguard/d1_rule_learning/promote.py` — promotes only if `hits >= 3 AND
      false_positives == 0`; persists to the `learned_rules` table `db.py` already provisioned
      in Phase 1.
- [x] `src/ledgerguard/d1_rule_learning/compile.py` — turns a spec back into an L1-rule-shaped
      callable (`matcher.run_l1`'s new `extra_rules` parameter), interpreting data, never
      `eval`/`exec`-ing code.
- [x] `eval/rule_learning.py` (`make d1`) runs three sequential batches and records LLM
      invocation rate per batch.
- [x] LLM invocation rate measured across 3 batches and **falls**: 17.1% → 15.0% → 11.2%.
- [x] Precision on auto-posted entries does **not** degrade: 88.6% → 97.8% → 97.8% (holds, and
      in fact improves once rules are promoted).
- [x] Cost per 1,000 records falls in step: $4.91 → $4.31 → $3.23 (directly proportional to
      invocation count by construction -- see `eval/rule_learning.py`'s module docstring for the
      pricing-table-based estimate this uses, since no live token counter is available in this
      environment).
- [x] Chart saved to `eval/output/invocation_decay.png` (gitignored like `reliability_diagram.png`
      before it -- regenerated by `make d1`, not committed).
- [x] Tests, matching phases.md's own three named cases exactly, plus direct unit coverage of
      propose/validate/compile: a rule with a single false positive is rejected even with enough
      hits; a promoted rule resolves at L1 on the next batch (invocation count for that pattern
      provably drops from 3/3 to 0/3 in a controlled unit test); the registry survives a restart
      (saved, connection closed, reopened, reloaded). `make test` passes (129 tests, 6 new).

**Why this needed its own generated dataset, not `data/samples`.** Checked directly before
writing any batch-driver code: `data/samples`'s realistic, mostly-easy 300-line fixture contains
only 2 bank lines total that both (a) escalate past L1 and (b) have one clean, single-order
correct answer to learn -- nowhere near the `hits >= 3` bar across 3 sequential batches.
`eval/rule_learning.py` generates its own dataset (`data.generator.generate`, seed 42, 900 bank
lines, `chaos_min_count=60`) specifically so there is enough repeated volume of the one
genuinely-learnable pattern in this project's design (see below) to actually exercise the loop --
a disclosed methodology choice, not a hidden one. Batches are a fixed-seed shuffle of train+
validation bank lines, not a chronological sort by `value_date`: sorting by date was tried first
and rejected because `SPLIT_SETTLEMENT`'s value_date (the *latest* group member's date, an order
statistic) skews toward the end of the date range as group size grows, which concentrated that
category's own escalations into the last batch and confounded "the rate changed" with "the batch
composition changed" rather than "a rule was learned" -- documented in full in `BROKE.md`.

**What is actually learnable here, and why.** L1's built-in `rule_single_payment_net_match`
already ignores narration entirely -- it resolves on exact net-amount + a fixed [2,4]-day window.
Every bank line in this dataset that escalates *and* has one clean correct answer turns out to
escalate for exactly one structural reason: `DATE_SKEW_BOUNDARY`'s "just past the window" half
(this project's own T+5 settlement case, generated on purpose per phases.md Phase 2). A learned
rule therefore widens that window, scoped to the same counterparty (narration token) the
resolved exception came from -- a realistic reconciliation pattern ("this merchant settles a few
days later than most"), not an artifact invented to make the demo work.

**A real, pre-existing bug found while investigating (Phase 2/3-era, not fixed here -- out of
this phase's scope, honestly logged instead):** every `SPLIT_SETTLEMENT` bank line in
`data/samples` turns out to escalate, not resolve, because the category's generator draws each
group member's `captured_at` independently across the full 0-120 day range with no requirement
that they cluster together -- so members can be far enough apart in time that some of them fall
outside `rule_subset_sum_split_settlement`'s own date-tolerance filter and never even enter its
candidate pool. Confirmed this predates Phase 7's subset-sum ambiguity fix (identical resolution
counts with the pre-fix rule code). Full account in `BROKE.md`.
